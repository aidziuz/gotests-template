...
func (p *Parser) Parse(srcPath string, files []models.Path) (*Result, error) {
	...
	funcs, constr := p.parseFunctions(fset, f, fs)
	return &Result{
		Header: &models.Header{
...
func (p *Parser) parseFunctions(fset *token.FileSet, f *ast.File, fs []*ast.File) ([]*models.Function, *models.Function) {
	ul, el := p.parseTypes(fset, fs)
	var funcs []*models.Function
	var constr *models.Function
	for _, d := range f.Decls {
		fDecl, ok := d.(*ast.FuncDecl)
		if !ok {
			continue
		}
		fun := parseFunc(fDecl, ul, el)
...
func parseFunc(fDecl *ast.FuncDecl, ul map[string]types.Type, el map[*types.Struct]ast.Expr) *models.Function {
	...
	for _, fi := range fs {
		...
		if strings.Contains(f.Name, "New") && fi.HasNoPackage() && len(fs) == 1 {
